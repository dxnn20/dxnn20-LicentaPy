...
from ResNet152Model import ResNet152Model  # Import your model class
...
model = ResNet152Model(num_classes).to(device)
...
for class_name in class_names:
    if class_name in total_per_class and total_per_class[class_name] > 0:
        ...
        print(f"{class_name}: {class_acc * 100:.2f}% ({correct_per_class.get(class_name, 0)}/{total_per_class[class_name]})")
    else:
...
plt.savefig('resnet152_confidence_distribution.png')
plt.show()
